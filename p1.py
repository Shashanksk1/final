    ...
    with sr.Microphone() as source:
        ...
        while True:
            ...
            try:
                ...
                playsound("currency_detection.mp3")
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            with sr.Microphone() as sourceA:
                for i, det in enumerate(pred):  # per image
                    ...
                    try:
                        # Use a speech recognition API to convert audio to text
                        text_ob = rec.recognize_google(audio)
                        print(f"You said: {text_ob}")
                        # Check if the text matches the termination phrase
                        if text_ob.lower() == "stop":
            ...
            if 'no detections' in k:
                text_obname="coundnt detect "
                print("coundnt detect ")
                myobj = gTTS(text=text_obname, lang=language, slow=False)
                myobj.save("currency_detection.mp3")
            else:
                text_obname="detected"+k[11:k.find('<')]
                myobj = gTTS(text=text_obname, lang=language, slow=False)
                myobj.save("currency_detection.mp3")
...
async def main():
    with sr.Microphone() as source:
        while True:
            bot = Chatbot(cookie_path='./coockies.json')
            print("Listening...")
            audio = rec.listen(source)
            try:
                # Use a speech recognition API to convert audio to text
                ...
                if text.lower() == "stop":
                    playsound('stoping_device.mp3')
                    break
                elif 'object' in text or 'obe' in text:
                    od()
                    continue
                elif 'currency' in text or 'cur' in text:
                    curd()
                    continue
                elif 'color' in text or 'col' in text:
                    cd()
                    continue
